      ...
      <div class="container col border  ps-4 rounded mt-4 ">
                ...
                <div class="row shadow-lg  rounded border mt-5 ">
                        ...
                        <div class="col-5 ps-4 pt-2">
                            ...
                            <a href="{{url('adminPages/DrugType/AddNewDrugType')}}" class="btn btn-outline-secondary " role="button" >+ Add New Drug Type</a>
                        </div>
                        <div class=" col-7 p-2 ">
...
@foreach($drugtypes as $drugtype)
    <tr>
        <th scope="row">{{ ++$i }}</th>
        <td>{{$drugtype->id}}</td>
        <td> {{$drugtype->catagory->catagory_name}}</td>
        <td>{{ $drugtype->unit->unit_name }}</td>
        
        <td>{{ $drugtype->brand_name }}({{ $drugtype->generic_name }})</td>
        <td>{{ $drugtype->dosage }}</td>
        <td>{{ $drugtype->price }}</td>
        ...
        <a href="{{url('adminPages/DrugType/'.$drugtype->id.'/delete')}}" class="btn btn-danger">Delete</a>
           </a>
        </td>
    </tr>
